 ...
 * @version 1.0
 * Предназначен для рассчета прибыли по вкладу.
...
public class DepositCalculator {
    ...
    private termSelect paymentType;
     ...
     */
    public DepositCalculator(long startAmount, int placementPeriod, termSelect periodType, double interestRate, double taxRate, boolean capitalisation, LocalDate startDate, termSelect paymentType) {
     ...
     */
    public DepositCalculator(long startAmount, int placementPeriod, termSelect periodType, double interestRate, double taxRate, boolean capitalisation, LocalDate startDate, ArrayList<BalanceAction> pushMoney, ArrayList<BalanceAction> popMoney, termSelect paymentType) {
        this.startAmount = startAmount;
        this.placementPeriod = placementPeriod;
        this.periodType = periodType;
        this.interestRate = interestRate;
    ...
    /**
     * Главная функция для расчета вклада
     * */
    public void calculate() {
        if (paymentType == termSelect.day) dayPayments();
        else if (paymentType == termSelect.month) monthPayments();
        else if (paymentType == termSelect.year) yearPayments();
    }
    ...
    private void dayPayments() {
        ...
        while (currentDate.isBefore(endDate)) {
            ...
            checkPushPopBalanceLists(currentDate);
            currentDate = currentDate.plusDays(1);
        }
        preparedResults();
    }

    private void monthPayments() {
        LocalDate currentDate = startDate.plusDays(1);
        resultDepositAmount = BigDecimal.valueOf(startAmount);
        LocalDate endDate = startDate.plusDays(placementPeriod);
    ...
    private void getDebitDaysCount() {
        LocalDate end_date;
        if (periodType == termSelect.year) end_date = startDate.plusYears(placementPeriod);
        else if (periodType == termSelect.month) end_date = startDate.plusMonths(placementPeriod);
